...
import type { GptImageEditPayload } from "~/types";

export const useWavespeedGptImagePayloadMapper = () => {
   ...
   * @returns {Promise<GptImageEditPayload>} The formatted payload
   */
  const buildPayload = async (): Promise<GptImageEditPayload> => {
   ...
   * @param {GptImageEditPayload} payload - The generation payload
   * @returns {Promise<string>} The Task ID
   */
  const submitTask = async (payload: GptImageEditPayload): Promise<string> => {
    const response: any = await $fetch("/api/gpt-image/generate", {
      method: "POST",
      body: payload,
    });
